...
void matchDescriptors(std::vector<cv::KeyPoint> &kPtsSource,
  ...
  if (matcherType.compare("MAT_BF") == 0) {
    ...
    if (descSource.type() != CV_32F) {
      // OpenCV bug workaround : convert binary
    ...
    cout << "Error: Unknown matcher type: " + matcherType;
    exit(1);
  }
  ...
  if (selectorType.compare("SEL_NN") == 0) {
    ...
    cout << "Error: Unknown selector type: " + selectorType;
    exit(1);
  }
}
...
// Harris keypoint detection, taken from solution.
void detKeypointsHarris(vector<cv::KeyPoint> &keypoints, cv::Mat &img,
  ...
  if (bVis) {
    ...
    cv::Mat visImage = dst_norm.clone();
    cv::drawKeypoints(img, keypoints, visImage, cv::Scalar::all(-1),
                      cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
...
void detKeypointsModern(vector<cv::KeyPoint> &keypoints, cv::Mat &img,
  ...
  if (detectorType.compare("FAST") == 0) {
    ...
    cout << "Internal error: Unknown detector type: " + detectorType << endl;
    exit(1);
  }
  double t = (double)cv::getTickCount();
